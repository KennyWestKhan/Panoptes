<template>
  <div>
    <v-card class="mx-auto" max-width="344">
      <v-img :src="getImgUrl()" height="200px" max-height="200" aspect-ratio="16/9"></v-img>
...
export default {
  ...
  methods: {
    ...
    getImgUrl() {
      const { fileId } = this.document
      let imgsrc;
      if(fileId) {
        const { filename } = fileId.metadata; 
        imgsrc = require(`../../../server/uploads/${filename}`);
        console.log({filename})
      } else {
        imgsrc = "../assets/empty-docs-icon.png";
      }
      return imgsrc;
    },
